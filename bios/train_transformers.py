...
def evaluate(mode, model, dl, epoch, args):
    ...
    for batch_idx, batch in enumerate(tqdm(dl)):
        ...
        with torch.no_grad():
            if args.dataparallel:
                output = model.module(input_ids=input_ids, attention_mask=attention_mask)
            else:
...
def main():
    ...
    parser.add_argument("--model_name_or_path", type=str, default="albert-base-v2")# bert-base-uncased,roberta-base,albert-base-v2
    ...
    def preprocess_function(examples):
        # Tokenize the texts
        args = [examples["bio"]]
        result = tokenizer(*args, padding="max_length", max_length=args.max_seq_len, truncation=True)
        return result
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        args.model_name_or_path, state_dict=torch.load(args.model_recover_path), num_labels=len(mapping))
    ...
    with open(os.path.join(args.output_dir, "result-transformers.json"), "a") as f_out:
