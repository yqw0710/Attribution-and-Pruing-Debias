...
def eval_acc(model, dl, args,importance_set=None,ratio=0,prune_choice="predict"):
    model.eval()
    print("running eval")
...
def main():
    ...
    model = AutoModelForSequenceClassification.from_pretrained(args.model_name_or_path, state_dict=model_state_dict,
    ...
    head_bias_path = os.path.join(args.output_dir, "head_bias_importance_attr_transformers.json")
    head_predicte_path = os.path.join(args.output_dir, "head_predicte_importance_attr_transformers.json")
    ...
    for p_ratio in protected_ratio:  # Percentage of traversal pruning
        ...
        log_dict = eval_model(model, eval_loader, args,importance_set,ratio=p_ratio,prune_choice=args.prune_choice)
        ...
        with open(os.path.join(args.output_dir, "biasinbios-eval-transformers.json"), "a") as f_out:
