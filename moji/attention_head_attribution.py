...
def process_data(dataset,is_debias=False):
    ...
    if is_debias:
        for elem in dataset:
            ...
            original_text_1="An African American says: "+original_text
            original_text_0 ="A Standard American says: "+original_text

            sentiment.append(elem["sentiment"])
            text.append(original_text_1)
            race.append(1)
            sentiment.append(elem["sentiment"])
            text.append(original_text_0)
            race.append(0)
    else:
        for elem in dataset:
            sentiment.append(elem["sentiment"])
    ...
    for batch_idx, batch in enumerate(tqdm(example_dataloader)):
        ...
        with torch.no_grad():
            ...
            if is_debias:
                ...
                if maxes[0] != labels[0] and maxes[1] != labels[1]:
                    continue
                if abs(logits[0][labels[0].item()] - logits[1][labels[1].item()]) <= 0.08:
                    continue
            else:
                if maxes[0] != labels: # Find the correct classification
                    continue
                if logits[0][labels[0].item()] < 0.9:
                    continue
        ...
        if is_debias == True: # bias attribution
            ...
            for tar_layer in range(0, num_layer):
                ...
                for j_batch in range(args.num_batch):  # num_batch:"Num batch of an example."
                    ...
                    tar_prob, grad = model(input_ids=input_ids, get_which="att", token_type_ids=token_type_ids, attention_mask=attention_mask, labels=labels,
                                           tar_layer=tar_layer, tmp_score=one_batch_att, pred_label=labels[0])  # one_batch_att传给tmp_score
                    grad = grad.sum(dim=0)
...
def main():
    ...
    parser.add_argument("--model_file",default="../model_save/moji/bert/original/model.2.bin",
                        type=str, help="The model file which will be evaluated.")
    parser.add_argument("--max_seq_length", default=128, type=int,
                        help="The maximum total input sequence length after WordPiece tokenization. \n"
                             "Sequences longer than this will be truncated, and sequences shorter \n"
                             "than this will be padded.")
    parser.add_argument("--val_path", type=str, default="../data/moji/devmoji.pkl")
    parser.add_argument("--bias_path", type=str, default="../data/moji/devmoji.pkl")
    ...
    eval_loader = DataLoader(dataset=val_dataset, batch_size=1, shuffle=True)
