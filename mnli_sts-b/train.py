...
from pytorch_pretrained_bert.modeling_orig import BertForSequenceClassification
from pytorch_pretrained_bert.optimization import BertAdam
from classifier_processer import InputExample, InputFeatures,DataProcessor, MrpcProcessor, MnliProcessor, RteProcessor, ScitailProcessor, ColaProcessor, SstProcessor, QqpProcessor, QnliProcessor, WnliProcessor, StsProcessor
...
# Data preprocessing, label_list:['test_matched', 'test_mismatched', 'diagnostic']
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if example.text_b:
            ...
            # 修改“tokens_a”和“tokens_b”，使总长度小于指定长度
...
def main():
    ...
    parser.add_argument("--output_dir",
                        default='../model_save/mnli/bert/original',
                        type=str,
    ...
    parser.add_argument("--local_rank",
                        type=int,
                        default=-1,
                        help="local_rank for distributed training on gpus")# 当前进程所用的是哪块卡，当前进程index
    ...
    parser.add_argument('--gradient_accumulation_steps',
                        type=int,
                        default=1,
                        help="Number of updates steps to accumulate before performing a backward/update pass.") #在执行向后/更新过程之前要累积的更新步骤数
    ...
    # When local_rank = -1 or 0, we consider it to be the master process.
    ...
    processor = processors[task_name]() # mnli
    num_labels = num_labels_task[task_name] # 2
    label_list = processor.get_labels() # ['test_matched', 'test_mismatched', 'diagnostic']
    ...
    # Prepare model
    if (args.model_file is None) or len(args.model_file) == 0:
        ...
        if not Path(args.model_file).exists():
            ...
            sys.exit(0)
        logger.info(
            "***** Recover model: {0} *****".format(args.model_file))
        model = BertForSequenceClassification.from_pretrained(
            args.model_name_or_path, state_dict=torch.load(args.model_file), num_labels=num_labels)
    if args.fp16:
        model.half()
    model.to(device)
    ...
    # Prepare optimizer
    param_optimizer = list(model.named_parameters()) # name和param。
    ...
    all_exist = True
    for i_epoch in range(1, int(args.num_epochs)+1):
    ...
    if args.do_train and (not all_exist):
        ...
        model.train()
        for i_epoch in trange(1, int(args.num_epochs)+1, desc="Epoch"): # with a progress bar
            tr_loss = 0
            ...
            model_to_save = model.module if hasattr(
                model, 'module') else model  # Only model_save the model it-self
            output_model_file = os.path.join(
                args.output_dir, "model.{0}.bin".format(i_epoch))
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        seg_result_dict = {}
        for i_epoch in trange(1, int(args.num_epochs)+1, desc="Epoch"):
            logger.info("***** CUDA.empty_cache() *****")
            torch.cuda.empty_cache()
            del model
