...
def pred_argmax(out):
    # Get the index of the element of the array with the largest value in one of its dimensions.
...
class MnliProcessor(DataProcessor):
    ...
    def get_debias_examples(self, data_dir, segment='debias_gender'):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, segment + ".tsv")), segment,is_debias=True)
    ...
    def _create_examples(self, lines, set_type, is_test=False,is_debias=False):
        """Creates examples for the training and dev sets."""
        examples = []
        for (i, line) in enumerate(lines):
            if i == 0:
