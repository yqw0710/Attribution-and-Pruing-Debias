...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, tmp_score=None, att_head_mask=None):
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, tmp_score=None, att_head_mask=None):
        self_output, att_score = self.self(input_tensor, attention_mask, tmp_score, att_head_mask)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, tmp_score=None, att_head_mask=None):
        attention_output, att_score = self.attention(hidden_states, attention_mask, tmp_score, att_head_mask)
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, 
                    tar_layer=None, tmp_score=None, att_head_mask=None):
        all_encoder_layers = []
        all_att_score = []
        ...
        if att_head_mask is None:
            ...
            # mask some attention heads
            for layer_index, layer_module in enumerate(self.layer):
                att_head_mask_one_layer = att_head_mask[layer_index]
                hidden_states, att_score = layer_module(hidden_states, attention_mask, tmp_score, att_head_mask_one_layer) 
                all_att_score.append(att_score)
                if output_all_encoded_layers:
                    all_encoder_layers.append(hidden_states)
            if not output_all_encoded_layers:
                all_encoder_layers.append(hidden_states)
            return all_encoder_layers, all_att_score
...
class PreTrainedBertModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name, state_dict=None, cache_dir=None, *inputs, **kwargs):
        ...
        # Instantiate model.
        model = cls(config, *inputs, **kwargs) # cls() 表示实例化
...
class BertForSequenceClassification(PreTrainedBertModel):
    ...
    def forward(self, input_ids, get_which="att", token_type_ids=None, attention_mask=None, labels=None, 
                        tar_layer=None, tmp_score=None, att_head_mask=None, pred_label=None):
        ...
        if get_which=="log":
            _, pooled_output, att_score = self.bert(
                input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False,att_head_mask=att_head_mask)
            pooled_output = self.dropout(pooled_output)
            logits = self.classifier(pooled_output)
            return logits
        if (get_which != "att" and get_which != "res") or labels is None:
            raise ValueError(
                "The argument get_which should be [att] or [res]")
        if get_which == "att": # No pruning in train
            _, pooled_output, att_score = self.bert(
                input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False,
                tar_layer=tar_layer, tmp_score=tmp_score)
            pooled_output = self.dropout(pooled_output)
            ...
            if tmp_score is None:
                return att_score, logits   # baseline logits / loss
            else:
                if len(logits[0])==1:
        ...
        elif get_which == "res": # Pruning at test
            _, pooled_output, att_score = self.bert(
                input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False, att_head_mask=att_head_mask)
            pooled_output = self.dropout(pooled_output)
            logits = self.classifier(pooled_output)
